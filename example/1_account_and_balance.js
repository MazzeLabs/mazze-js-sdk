/* eslint-disable */
const { ProxyProvider, Mazzy } = require('../src'); // require('mazze-js-sdk');
...
async function getAccountBalance() {
  const balance = await proxyProvider.getBalance('mazzetest:aar8jzybzv0fhzreav49syxnzut8s0jt1a1pdeeuwb');

  console.log(balance); // "4999998839889983249999999950307784"
  console.log(Mazzy(balance).toGMazzy()); // "4999998839889983249999999.950307784"
  console.log(Mazzy(balance).toMAZZE()); // "4999998839889983.249999999950307784"
}

function encryptAndDecryptPrivateKeyAccount() {
  // create Account by privateKey
  const account = proxyProvider.wallet.addPrivateKey('0x46b9e861b63d3509c88b7817275a30d22d62c8cd8fa6486ddee35ef0d8e0495f');
  console.log(proxyProvider.wallet.has(account.address)); // true

  proxyProvider.wallet.delete(account.address);
  console.log(proxyProvider.wallet.has(account.address)); // false
  ...
  const decryptedAccount = proxyProvider.wallet.addKeystore(keystore, 'password');
  console.log(decryptedAccount);
  /*
  PrivateKeyAccount {
    address: 'mazzetest:aar8jzybzv0fhzreav49syxnzut8s0jt1a1pdeeuwb',
...
function genRandomAccount() {
  const randomAccount = proxyProvider.wallet.addRandom();
  console.log(randomAccount);
  /*
  PrivateKeyAccount {
    address: 'mazzetest:aapwgme763vhz9jw38nnbdjg1bzx1g8n3e6mtfmm6r',
  ...
  console.log(proxyProvider.wallet.addRandom()); // different account
  /*
  PrivateKeyAccount {
    address: 'mazzetest:aas95jhnaf9m2z019wp8rfz1fgje63tc268aaeyaup',
