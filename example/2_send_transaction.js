/* eslint-disable */
const { ProxyProvider, Transaction, Mazzy } = require('../src'); // require('mazze-js-sdk');
...
const accountAlice = proxyProvider.wallet.addPrivateKey('0xa816a06117e572ca7ae2f786a046d2bc478051d0717bf5cc4f5397923258d393', 1);
const addressBob = 'mazzetest:aatm5bvugvjwdyp86ruecmhf5vmng5ysy2pehzpz9h';
...
async function sendTransactionSimple() {
  console.log(await proxyProvider.getBalance(accountAlice)); // 96283101378499979000
  console.log(await proxyProvider.getBalance(addressBob)); // 0

  // alice send some MAZZE to bob
  const txHash = await proxyProvider.sendTransaction({
    from: accountAlice.address, // `proxyProvider.wallet.has(accountAlice.address)` must be true
    to: addressBob,
    value: Mazzy.fromMAZZE(0.1), // 0.1 MAZZE = 100000000000000000 Mazzy
  });
  ...
  const transaction = await proxyProvider.getTransactionByHash(txHash);
  ...
  /*
  {
    ...
    "to": "MAZZETEST:TYPE.USER:AATM5BVUGVJWDYP86RUECMHF5VMNG5YSY2PEHZPZ9H"
  }
   */

  const receipt = await proxyProvider.getTransactionReceipt(txHash);
  ...
  */
  ...
  console.log(await proxyProvider.getBalance(accountAlice)); // 96183080378499979000
  console.log(await proxyProvider.getBalance(addressBob)); // 0
}
...
async function sendTransactionComplete() {
  const to = addressBob;
  const value = Mazzy.fromGMazzy(100); // 100 GMazzy = 100000000000 Mazzy
  const estimate = await proxyProvider.estimateGasAndCollateral({ to, value });

  const status = await proxyProvider.getStatus();
  const epochNumber = await proxyProvider.getEpochNumber();
  const gasPrice = await proxyProvider.getGasPrice();
  const nonce = await proxyProvider.getNextNonce(accountAlice.address);
...
async function signAndSendTransactionManual() {
  const options = {
    nonce: await proxyProvider.getNextNonce(accountAlice.address), // or just `getNextNonce(accountAlice)`
    gasPrice: 1000000000,
    gas: 21000,
  ...
  /*
    Transaction {
    from: 'MAZZETEST:TYPE.USER:AATM5BVUGVJWDYP86RUECMHF5VMNG5YSY2PEHZPZ9H',
    nonce: 6,
    gasPrice: 1000000000,
    gas: 21000,
    to: 'MAZZETEST:TYPE.USER:AATM5BVUGVJWDYP86RUECMHF5VMNG5YSY2PEHZPZ9H',
  ...
  const txHash = await proxyProvider.sendRawTransaction(transaction.serialize()); // send hex string here
...
async function subscribeTransaction() {
  // NO await !!!
  const pendingTransaction = proxyProvider.sendTransaction({
    from: accountAlice,
