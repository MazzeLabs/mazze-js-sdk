...
const { ProxyProvider, sign, format } = require('../src'); // require('mazze-js-sdk');
...
async function getGenesisBlock() {
  const genesisBlock = await proxyProvider.getBlockByEpochNumber(0);
  ...
  genesisBlock {
    ...
    "miner": "MAZZETEST:TYPE.USER:AAJAAAAAAAAAAAAAAAAAAAAAAAAAAAABAAV2WGH9U7",
    "nonce": "0x0",
  ...
  */
  ...
  const parent = await proxyProvider.getBlockByHash(genesisBlock.parentHash);
...
async function findInformationAboutTransaction() {
  ...
  const block = await proxyProvider.getBlockByHash(transaction.blockHash);
  console.log(block);
