...
class Transaction {
   ...
   * @param {string|number} [options.gasPrice] - The price of gas for this transaction in mazzy.
   ...
   * @param {string|number} [options.value] - The value transferred for the transaction in mazzy, also the endowment if it’s a contract-creation transaction.
  ...
  encode(includeSignature) {
    ...
    if (this.type === TRANSACTION_TYPE_LEGACY) { // legacy transaction
      const { nonce, gasPrice, gas, to, value, storageLimit, epochHeight, chainId, data, v, r, s } = mazzeFormat.signTx(this);
      ...
      const { nonce, maxPriorityFeePerGas, maxFeePerGas, gas, to, value, storageLimit, epochHeight, chainId, data, v, r, s } = mazzeFormat.sign1559Tx(this);
      const accessList = this.encodeAccessList();
