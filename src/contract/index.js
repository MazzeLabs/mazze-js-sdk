...
class Contract {
   ...
   * @param {import('../ProxyProvider').ProxyProvider} proxyProvider - ProxyProvider instance.
   * @return {object}
   *
   * @example
   * > const contract = proxyProvider.Contract({ abi, bytecode, address });
   {
   ...
   * @example
   * > const contract = proxyProvider.Contract({
   address: 'mazzetest:achc8nxj7r451c223m18w2dwjnmhkd6rxa2gc31euw',
   abi: [
      {
        type: 'function',
   ...
   * > contract.address
   "mazzetest:achc8nxj7r451c223m18w2dwjnmhkd6rxa2gc31euw"
   ...
   * > await contract.transfer('0x19c742cec42b9e4eff3b84cdedcde2f58a36f44f', 10000).sendTransaction({
     from: 'mazzetest:aak2rra2njvd77ezwjvx04kkds9fzagfe6d5r8e957',
   });
   0x2055f3287f1a6ce77d91f5dfdf7517a531b3a560fee1265f27dc1ff92314530b

   * > transaction = await proxyProvider.getTransactionByHash('0x2055f3287f1a6ce77d91f5dfdf7517a531b3a560fee1265f27dc1ff92314530b');
   ...
   * > receipt = await proxyProvider.getTransactionReceipt('0x2055f3287f1a6ce77d91f5dfdf7517a531b3a560fee1265f27dc1ff92314530b');
   ...
   */
  constructor({ abi, address, bytecode, decodeByteToHex }, proxyProvider) {
    if (abi.length > 0 && typeof abi[0] === 'string') {
      abi = parseABIFromString(abi);
    }
    _feedAddressNetId(abi, proxyProvider);
    ...
    if (address) {
      this.address = (proxyProvider && proxyProvider.networkId) ? proxyProvider._formatAddress(address) : address; // XXX: Create a method named `address` in solidity is a `ParserError`
    }

    // constructor
    this.constructor = new ContractConstructor(lodash.first(abiTable.constructor), bytecode, this, proxyProvider);

    // method
    const methodArray = lodash.map(abiTable.function, fragment => new ContractMethod(fragment, this, proxyProvider));
    lodash.forEach(lodash.groupBy(methodArray, 'name'), (array, name) => {
      this[name] = array.length === 1 ? lodash.first(array) : new ContractMethodOverride(array, this, proxyProvider);
    ...
    // event
    const eventArray = lodash.map(abiTable.event, fragment => new ContractEvent(fragment, this, proxyProvider));
    lodash.forEach(lodash.groupBy(eventArray, 'name'), (array, name) => {
      this[name] = array.length === 1 ? lodash.first(array) : new ContractEventOverride(array, this, proxyProvider);
...
function _feedAddressNetId(abi, proxyProvider) {
  if (!abi || !proxyProvider || !proxyProvider.networkId) return;
  ...
  function _feedInfo(items = []) {
    for (const meta of items) {
      if (meta.type === 'address') {
        meta.networkId = proxyProvider.networkId;
      }
      if (meta.type === 'tuple') {
