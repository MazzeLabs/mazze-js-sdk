...
function supplementErrorInfo(object, payload) {
  ...
  if (object.message === 'Method not found' && payload.method === 'mazze_sendTransaction') {
    object.message = `${object.message} Can't find 'from' in mazze.wallet, check 'error.data' for detail`;
    object.data = 'Please use mazze.wallet.addPrivateKey() to add a account before call mazze.sendTransaction()';
  }
