...
class PendingTransaction {
   ...
   * @param {import('../ProxyProvider').ProxyProvider} proxyProvider
   * @param {function} func
   * @param {array} args
   * @return {PendingTransaction}
   */
  constructor(proxyProvider, func, args) {
   ...
   * @return {Promise<import('../rpc/types/formatter').Transaction|null>} See [ProxyProvider.getTransactionByHash](#ProxyProvider.js/getTransactionByHash)
   */
  async get({ delay = 0 } = {}) {
    await sleep(delay);
    const transactionHash = await this;
    return this.proxyProvider.getTransactionByHash(transactionHash);
  }
   ...
   * @return {Promise<import('../rpc/types/formatter').Transaction>} See [ProxyProvider.getTransactionByHash](#ProxyProvider.js/getTransactionByHash)
   */
   ...
   * @return {Promise<import('../rpc/types/formatter').TransactionReceipt>} See [ProxyProvider.getTransactionReceipt](#ProxyProvider.js/getTransactionReceipt)
   */
  async executed({ delta = 1000, timeout = 5 * 60 * 1000 } = {}) {
    ...
    const transactionHash = await this;
    for (let lastTime = startTime; lastTime < startTime + timeout; lastTime = Date.now()) {
      const receipt = await this.proxyProvider.getTransactionReceipt(transactionHash);
      if (receipt) {
  ...
  async confirmed({ delta = 1000, timeout = 30 * 60 * 1000, threshold = 1e-8 } = {}) {
    ...
    for (let lastTime = startTime; lastTime < startTime + timeout; lastTime = Date.now()) {
      ...
      const risk = await this.proxyProvider.getConfirmationRiskByHash(receipt.blockHash);
      if (risk <= threshold) {
        return receipt;
      }
