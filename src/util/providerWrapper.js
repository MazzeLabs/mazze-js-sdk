...
async function useProxyProviderPrepareTx(txInfo, callRPC) {
  if (!txInfo.chainId) {
    const { chainId } = await callRPC('mazze_getStatus');
  ...
  if (!txInfo.gas || !txInfo.storageLimit) {
    ...
    const { gasLimit, storageCollateralized } = await callRPC('mazze_estimateGasAndCollateral', txInfo);
    if (!txInfo.gas) {
      txInfo.gas = gasLimit;
    }
  ...
  if (!txInfo.nonce) {
    txInfo.nonce = await callRPC('mazze_getNextNonce', txInfo.from);
  }
  if (!txInfo.epochHeight) {
    txInfo.epochHeight = await callRPC('mazze_epochNumber');
  }
  return txInfo;
}

function wrapProxyProvider(provider) {
  if (typeof ethereum === 'undefined') {
    throw new Error('MetaMask is not installed!');
  }
  ...
  async function request(method, ...params) {
    if (method !== 'mazze_sendTransaction') {
      return originRequest(method, ...params);
    }
    let txInfo = params[0];
    txInfo = await useProxyProviderPrepareTx(txInfo, originRequest);
    const rawTx = await signWithMetaMask(txInfo);
    return originRequest('mazze_sendRawTransaction', rawTx);
  }
