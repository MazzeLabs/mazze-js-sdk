...
const HTTP_URL = 'http://main.mazze.io';
const WS_URL = 'ws://main.mazze.io/ws';

test('BaseProvider', async () => {
  const provider = providerFactory({});
  expect(provider.constructor.name).toEqual('BaseProvider');

  await expect(provider.call('mazze_epochNumber')).rejects.toThrow('BaseProvider.request not implement');

  expect(await provider.batch([])).toEqual([]);

  await expect(
    provider.batch([{ method: 'mazze_epochNumber' }, { method: 'NOT_EXIST' }]),
...
test.skip('HttpProvider', async () => {
  ...
  const result = await provider.call('mazze_epochNumber');
  expect(result).toMatch(/^0x[\da-f]+$/);

  const array = await provider.batch([{ method: 'mazze_epochNumber' }, { method: 'NOT_EXIST' }]);
...
test.skip('WebSocketProvider', async () => {
  ...
  const array = await provider.batch([{ method: 'mazze_epochNumber' }]);
  expect(array.length).toEqual(1);
  expect(array[0]).toMatch(/^0x[\da-f]+$/);

  const id = await provider.call('mazze_subscribe', 'epochs');
  await new Promise(resolve => provider.once(id, resolve));

  const promise = provider.call('mazze_epochNumber');
  provider.close();
  provider.close();
