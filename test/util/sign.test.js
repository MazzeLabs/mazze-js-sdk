...
test('publicKeyToAddress', () => {
  const address = format.address(publicKeyToAddress(format.hexBuffer(PUBLIC)), CONST.DEVNET_ID);
  expect(address).toEqual(ADDRESS);
});

test('privateKeyToAddress', () => {
  const address = format.address(privateKeyToAddress(format.hexBuffer(KEY)), CONST.DEVNET_ID);
  expect(address).toEqual(ADDRESS);
});
...
test('ecdsaSign and ecdsaRecover', () => {
  ...
  const address = format.address(publicKeyToAddress(publicKey), CONST.DEVNET_ID);
  expect(address).toEqual(ADDRESS);
});
