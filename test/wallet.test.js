const { ProxyProvider, sign, CONST } = require('../src');
...
const proxyProvider = new ProxyProvider({
  networkId: CONST.DEVNET_ID,
});

afterEach(() => {
  proxyProvider.wallet.clear();
});
...
test('set', async () => {
  ...
  proxyProvider.wallet.set(ADDRESS, account);
  expect(proxyProvider.wallet.has(ADDRESS)).toEqual(true);
  expect(proxyProvider.wallet.has(ADDRESS)).toEqual(true);

//   const ERROR_ADDRESS = `${ADDRESS.substr(0, 20)}${ADDRESS.substr(20).toUpperCase()}`;
//   expect(() => proxyProvider.wallet.set(ERROR_ADDRESS, account)).toThrow('checksum error');
});

test('set not BaseAccount', async () => {
  expect(() => proxyProvider.wallet.set(ADDRESS, null)).toThrow('not instance of Account');
});
...
test('set already exist', async () => {
  ...
  proxyProvider.wallet.set(ADDRESS, account);
  expect(() => proxyProvider.wallet.set(ADDRESS, account)).toThrow('already has account');

  proxyProvider.wallet.delete(ADDRESS);
  proxyProvider.wallet.set(ADDRESS, account);
  expect(proxyProvider.wallet.has(ADDRESS)).toEqual(true);
});

test('addPrivateKey', async () => {
  const account = proxyProvider.wallet.addPrivateKey(PRIVATE_KEY, CONST.DEVNET_ID);
  expect(proxyProvider.wallet.has(ADDRESS)).toEqual(true);
...
test('addKeystore', async () => {
  const account = proxyProvider.wallet.addKeystore(KEYSTORE, PASSWORD, CONST.DEVNET_ID);
  expect(proxyProvider.wallet.has(ADDRESS)).toEqual(true);
...
test('addRandom', async () => {
  const account1 = proxyProvider.wallet.addRandom();
  const account2 = proxyProvider.wallet.addRandom();
...
test('addRandom same entropy', async () => {
  ...
  const account1 = proxyProvider.wallet.addRandom(entropy);
  const account2 = proxyProvider.wallet.addRandom(entropy);
...
test('account.encrypt', () => {
  const account1 = proxyProvider.wallet.addPrivateKey(PRIVATE_KEY, CONST.DEVNET_ID);

  const keystore = account1.encrypt(PASSWORD);
  const account2 = account1.constructor.decrypt(keystore, PASSWORD, CONST.DEVNET_ID);
